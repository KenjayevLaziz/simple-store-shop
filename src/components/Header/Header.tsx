...
export default function Header() {
  ...
  const {likeCount}=useLikedProducts()
    ...
    <div className="dark:bg-[#020817] bg-white text-white w-full fixed   mx-auto top-0 left-0 right-0 z-50 border-b-white">
      ...
      <nav className="flex max-w-[450px] w-full justify-between">
        <ul className="dark:text-white text-black flex items-center justify-between max-w-[300px] w-full">
          ...
          <Link to="/">
            <li className="text-sm font-medium hover:underline capitalize cursor-pointer">home</li>
          </Link>
          <Link to="/">
            <li className="text-sm font-medium hover:underline capitalize cursor-pointer">products</li>
          </Link>
          <Link to="/category">
            <li className="text-sm font-medium hover:underline capitalize cursor-pointer">categories</li>
          </Link>
        </ul>
      </nav>
      ...
      <div className="max-w-[400px] w-full flex justify-end">
        <ul className="flex items-center gap-4 max-w-[300px] w-full justify-between">
          ...
          >
           ...
           {likeCount?<Link to="/wislist">
              <div className="relative cursor-pointer w-[45px] h-[45px] dark:hover:bg-[#1E293B] hover:bg-[#cfc6c6] rounded-sm flex items-center justify-center">
                ...
                <div className="absolute bg-[#3576DF] right-0 rounded-full text-[14px] w-5 h-5 top-0 flex justify-center items-center">{likeCount}</div>
            </div>
            </Link> :<Link to="/wislist">
            ...
            </Link>}
          {
            ...
            totalItems?<Link to="/cart">
              <div className="relative cursor-pointer w-[45px] h-[45px] dark:hover:bg-[#1E293B] hover:bg-[#cfc6c6] rounded-sm flex items-center justify-center">
                <ShoppingCart className="dark:hover:bg-[#1E293B] dark:text-white text-black" />
                <div className="absolute bg-[#3576DF] right-0 rounded-full text-[14px] w-5 h-5 top-0 flex justify-center items-center">{totalItems}</div>
            </div>
            </Link> :<Link to="/cart">
              <div className="cursor-pointer w-[45px] h-[45px] dark:hover:bg-[#1E293B] hover:bg-[#cfc6c6] rounded-sm flex items-center justify-center">
                <ShoppingCart className="dark:hover:bg-[#1E293B] dark:text-white text-black" />
          ...
          }
          {
            user?
              <div className="relative w-full">
                <Button onClick={()=>{toggleMenu()}}   className="rounded-full w-[50px] h-[50px] flex justify-center items-center bg-[yellow] cursor-pointer text-black">
              <span className="text-black uppercase"> {user?.name}</span>
              </Button>
              {open?<div className="absolute w-full border dark:*border-white top-[60px] left-0 bg-white dark:bg-[#020817] z-50 rounded-md">
                <h1 className="px-2 py-1.5 text-sm font-semibold capitalize">my acaunt</h1>
                <hr />
                <Link to="/profile">
                  <p onClick={()=>{toggleMenu()}}  className="px-2 py-1.5 cursor-pointer hover:bg-[#1E293B]">Profile</p>
                </Link>
                <Link to="/wislist">
                  <p onClick={()=>{toggleMenu()}}  className="px-2 py-1.5 cursor-pointer hover:bg-[#1E293B]">Wishlist</p>
                </Link>
                <Link to="/cart">
                  <p onClick={()=>{toggleMenu()}}  className="px-2 py-1.5 cursor-pointer hover:bg-[#1E293B]">Cart</p>
                </Link>
                <hr />
                  <p onClick={()=>{toggleMenu1()}}  className="px-2 py-1.5 cursor-pointer hover:bg-[#1E293B]">Logout</p>
              </div>:null}
              </div>
              :<Link to="/login">
                ...
                <span className="dark:text-amber-100 text-black capitalize">Login</span>
                </Button>
              </Link>
          }
        </ul>
